...
export function useAuth() {
  ...
  const { data: user, isLoading, error, refetch } = useQuery<User | null>({
    ...
    queryFn: async () => {
      try {
        const res = await apiFetch(api.auth.me.path, { credentials: 'include' });
        if (res.status === 401) {
  ...
  const requestOtpMutation = useMutation({
    mutationFn: async (phone: string) => {
      ...
      if (!res.ok) throw new Error(data.message || (lang === 'mg' ? "Tsy afaka nandefa kaody" : "Échec de l'envoi du code"));
      return data;
    },
    onSuccess: (_, phone) => {
  ...
  const loginMutation = useMutation({
    mutationFn: async (data: { phone: string; otp: string }) => {
      ...
      if (!valid) throw new Error(error || (lang === 'mg' ? "Nomerao tsy ara-dalàna" : "Numéro invalide"));
